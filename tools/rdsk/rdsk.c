...
#define ALIGN_UP(VALUE, ALIGN)                                                 \
  ((VALUE) % (ALIGN) > 0 ? (VALUE) + ((ALIGN) - (VALUE) % (ALIGN)) : (VALUE))
...
/* UTIL - FILETABLE */
static uint64_t filetable_create(char *name, uint64_t parent_index,
                                 uint64_t off, uint64_t size) {
  bool found = false;
  size_t i = 0;
  for (; i < g_filetable_size; i++) {
    if (g_filetable[i].used)
      continue;
    found = true;
    break;
  }
  if (!found) {
    rdsk_file_t *new_table =
        malloc(sizeof(rdsk_file_t) * (g_filetable_size + 16));
...
static void create(char *dir_path, char *out_path) {
  ...
  g_header->dirtable_offset = ALIGN_UP(
      g_header->nametable_offset + g_header->nametable_size, SECTION_ALIGNMENT);
  g_header->filetable_entry_size = sizeof(rdsk_dir_t);
  g_header->filetable_entry_count = g_filetable_size;
  g_header->filetable_offset =
      ALIGN_UP(g_header->dirtable_offset + g_header->dirtable_entry_count *
                                               g_header->dirtable_entry_size,
               SECTION_ALIGNMENT);
...
int main(int argc, char **argv) {
  ...
  case MODE_CREATE:
    ...
    break;
  default:
    printf("No mode was selected. Terminating\n");
    break;
  }
  if (errno != 0) {
